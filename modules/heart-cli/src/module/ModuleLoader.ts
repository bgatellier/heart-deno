    ...
    modulesPaths.forEach((modulePath: string) => {
      try {
        ...
        const requiredModuleDotenvVariables = Object.entries(
          parse(Deno.readTextFileSync(modulePath + this.ENVIRONMENT_VARIABLE_MODEL))
        )
        ...
        const missingModuleDotEnvVariables = requiredModuleDotenvVariables.filter(([variableName]) => Deno.env.get(variableName))
  
        // add the missing module dotenv variables to the missing list
        missingDotEnvVariables = [...missingDotEnvVariables, ...missingModuleDotEnvVariables]
