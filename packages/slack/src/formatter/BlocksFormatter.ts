...
import { formatGreenITBlocks } from "./GreenITStatisticsFormatter.ts";
import { formatLighthouseBlocks } from "./LighthouseStatisticsFormatter.ts";
import { formatObservatoryBlocks } from "./ObservatoryStatisticsFormatter.ts";
...
const createBlocks = (
  ...
  if (
    report.resultUrl !== undefined || report.isThresholdReached() !== undefined
  ) {
    ...
    if (report.isThresholdReached() !== undefined) {
      elements.push({
        ...
        text: report.isThresholdReached()
          ? `:white_check_mark: Threshold (${
            report.inputs.threshold ?? 0
          }) reached`
