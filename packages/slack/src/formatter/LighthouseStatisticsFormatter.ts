...
export const formatLighthouseBlocks = (
  ...
  const metricsBlocks: MrkdwnElement[] = [
    ...
    type: "mrkdwn",
    text: `*${report.result.audits[auditKey].title}*: ${
      report.result.audits[auditKey].displayValue ?? ""
    }`,
  }));
  ...
  const advicesBlocks: SectionBlock[][] = Object.values(
    ...
    .map((category) => {
      const auditsIds = category.auditRefs
        .filter((auditRef) =>
          !["metrics", "hidden"].includes(auditRef.group ?? "")
        )
        .map((auditRef) => auditRef.id);

      const advices = audits.filter(
        (audit) =>
          auditsIds.includes(audit.id) && audit.score !== null &&
          audit.score < 1,
      );
