...
import { MAX_TEXT_BLOCK_LENGTH } from "./BlocksFormatter.ts";
...
export const formatGreenITBlocks = (
  ...
  const metricsBlocks: MrkdwnElement[] = [
    {
      type: "mrkdwn",
      text: `*Page weight*: ${
        (report.result.responsesSize / 1000000).toFixed(2)
      } Mo`,
    },
    {
      type: "mrkdwn",
  ...
  const advicesBlocks: SectionBlock[][] = Object.values(
    ...
    .map((practice) => {
      ...
      const lines = practice.detailComment.split("<br>").filter((line) =>
        line.length > 0
      );
