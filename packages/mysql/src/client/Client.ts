...
import { type IMigrator, MikroORM } from "npm:@mikro-orm/core";
import { type MySqlDriver, SqlEntityManager } from "npm:@mikro-orm/mysql";
import databaseConfig from "../config/mikro-orm.config.ts";
...
export class MySQLClient {
  ...
  private async createReportEntity(
    report: GenericReport<Result>,
  ): Promise<ReportEntity<Result>> {
  ...
  private async getOrCreateServiceEntity(
    service: GenericReport<Result>["service"],
  ): Promise<ServiceEntity> {
    const em = await this.getOrCreateEntityManager();

    const serviceEntity = await em.findOne(ServiceEntity, {
      name: service.name,
    });
