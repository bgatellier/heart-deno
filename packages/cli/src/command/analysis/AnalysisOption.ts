...
import { isErrnoException } from "../../error/ErrnoException.ts";
import { readFile } from "../../filesystem/fs.ts";
import { snakeCaseToCamelCase } from "../../text/case.ts";
...
const ANALYSIS_OPTIONS: {
  [key in keyof Omit<AnalysisOptions, keyof CommonOptions>]-?: string;
} = {
  config: "config",
  threshold: "threshold",
...
export function createConfigOption(): Option {
  return new Option(
    `-${
      ANALYSIS_OPTIONS.config[0]
    }, --${ANALYSIS_OPTIONS.config} <${ANALYSIS_OPTIONS.config}>", "Path to the JSON configuration file`,
...
export function createThresholdOption(): Option {
  return new Option(
    `-${
      ANALYSIS_OPTIONS.threshold[0]
    }, --${ANALYSIS_OPTIONS.threshold} <${ANALYSIS_OPTIONS.threshold}>`,
...
export function createExceptListenersOption(): Option {
  return new Option(
    `-le, --${ANALYSIS_OPTIONS.exceptListeners} <${
      snakeCaseToCamelCase(ANALYSIS_OPTIONS.exceptListeners)
    }>`,
...
export function createOnlyListenersOption(): Option {
  return new Option(
    `-lo, --${ANALYSIS_OPTIONS.onlyListeners} <${
      snakeCaseToCamelCase(ANALYSIS_OPTIONS.onlyListeners)
    }>`,
