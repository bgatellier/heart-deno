...
import Ajv, { type AnySchema, type ErrorObject } from "npm:ajv";
...
import { EnvironmentError } from "../error/EnvironmentError.ts";
import type { PackageJsonModule } from "./PackageJson.ts";
...
export async function checkEnv(modulesPaths: string[]): Promise<void> {
  ...
  const schemas = schemasContent.map((content) =>
    JSON.parse(content) as AnySchema
  );
...
export async function initializeModules<M extends Module>(
  ...
  if (verbose) {
    logger.info(
      `Importing modules ${
        metadatas.map((metadata) => metadata.id).join(", ")
      }...`,
    );
  }
  ...
  return moduleIndexes.map((moduleIndex, i) =>
    moduleIndex.initialize(metadatas[i], verbose) as M
  );
}
...
async function getPaths(cwd: string): Promise<string[]> {
  ...
  const moduleIndex =
    (await import(packageJsonPath, { with: { type: "json" } }).catch(
      (error) => {
        return Promise.reject(error);
      },
