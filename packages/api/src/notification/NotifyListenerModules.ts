...
import type { FastifyReply, FastifyRequest } from "npm:fastify";

function notifyListenerModules(
  listenerModules: ModuleListenerInterface[],
  report: GenericReport<Result>,
): Promise<unknown[]> {
  const notifyListenerModulesPromises = listenerModules.map(
    (listenerModule) => {
    ...
    if (reply.statusCode >= 200 && reply.statusCode < 300) {
      ...
      if (except_listeners !== undefined) {
        listenerModulesResolved.push(
          ...listenerModules.filter((listenerModule) =>
            !except_listeners.includes(listenerModule.id)
          ),
        );
      } else if (only_listeners !== undefined) {
        listenerModulesResolved.push(
          ...listenerModules.filter((listenerModules) =>
